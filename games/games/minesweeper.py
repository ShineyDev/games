    ...
    def generate_numbers(self):
        ...
        for (row_index, row) in enumerate(self.hidden):
            for (column_index, cell) in enumerate(row):
                if (cell != "X"):
                    values = [self.hidden[row][column] for (row, column) in self.get_neighbors((row_index, column_index))]
...
class Minesweeper():
    ...
    def game(self):
        ...
        while (not flags == self.grid.mines):
            ...
            if (self.message):
                ...
                print("{0} mine{1} left".format(len(self.grid.mines) - len(flags), "s" if (len(self.grid.mines) - len(flags) != 1) else ""))
                print()
            ...
            if (coordinates.startswith("EVAL(")):
                ...
                pattern_string = r"^EVAL\(VA-(?P<column>[A-{0}])(?P<row>{1})\)$".format(string.ascii_uppercase[self.grid_size - 1], "|".join([str(i) for i in range(1, self.grid_size + 1)][::-1]))
    ...
    def parse_coordinates(self, coordinates: str) -> tuple:
        ...
        pattern = re.compile(r"^(?P<column>[A-{0}])(?P<row>{1})(?P<flag>F?)$".format(string.ascii_uppercase[self.grid_size - 1], "|".join([str(i) for i in range(1, self.grid_size + 1)][::-1])))
        match = re.match(pattern, coordinates)

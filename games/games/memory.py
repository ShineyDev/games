...
class Memory():
    ...
    def game(self):
        ...
        while (" " in self.known):
            ...
            self.known[(first_row * self.grid_size) + first_column] = characters[(first_row * self.grid_size) + first_column]
            ...
            while (True):
                ...
                if (self.valid_coordinates(coordinates)):
                    ...
                    self.known[(second_row * self.grid_size) + second_column] = characters[(second_row * self.grid_size) + second_column]
                    break
            ...
            if (not self.known[(first_row * self.grid_size) + first_column] == self.known[(second_row * self.grid_size) + second_column]):
    ...
    def parse_coordinates(self, coordinates : str):
        ...
        pattern = re.compile(r"(?P<column>[A-{0}])(?P<row>{1})".format(string.ascii_uppercase[self.grid_size - 1], "|".join([str(i) for i in range(1, self.grid_size + 1)][::-1])))
    ...
    def show_grid(self):
        ...
        print("       {0}".format("   ".join([character for character in string.ascii_lowercase][:self.grid_size])))
